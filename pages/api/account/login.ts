...
export default handler.use(withValidation(loginSchema)).post(async (req, res) => {
  await connectDB();
  const body = req.body;
  ...
  try {
    ...
    let login_user = { id: user._id, email: user.email, name: name, companyId: user.companyId, role: user.role };

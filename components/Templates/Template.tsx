      ...
      <div className="bg-white pt-4 pb-4 rounded-lg shadow-xl cursor-pointer border border-gray-300">
        <div className="relative h-20 flex items-center">
          <div className="absolute w-full flex justify-end mb-10 pr-4">
            <p className="uppercase font-bold text-gray-400">{moment(moment.utc(template.createdAt), "YYYYMMDD").fromNow()}</p>
          </div>
        ...
        <div className="pl-4 pr-2 mt-2 flex justify-between">
          ...
          <div className="flex justify-center items-center pr-2">
            <p className="uppercase font-extrabold text-green-500 text-sm">{`${template.tasksLength} ${
              template.tasksLength !== undefined && template?.tasksLength > 1 ? "tasks" : "task"
            }`}</p>
          </div>
        </div>
